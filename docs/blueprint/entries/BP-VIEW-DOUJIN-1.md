...
origin: "首次入庫於本次 block 1 開發（2026-08-26，worktree feat/doujin-view）；同日追加：
  ...
  資料」按鈕 + 來源徽章（見 BP-SVC-DOUJIN-1 的 metadata 抓取設計）；驗收（Playwright
  375/768/1280，10 項全過，含兩個 merge-blocking 項目）通過後又追加：exhentai 從封面牆
  移除（改走一般縮圖牆）、封面牆新增抓取狀態 ⚠ 徽章。"
---
...
- **`DoujinBookWall.vue`**——封面牆。呼叫 `GET /api/gallery/doujin/books?source=`，卡片顯示
  ...
  **抓取狀態徽章**（右上角 ⚠，2026-08-26 驗收後追加）：`needs_fetch_attention` 為
  `true`（`meta_fetch_status` 是 `blocked`/`not_found`/`network_error` 三者之一）才顯示
  ——這個布林值已經在封面牆那次批次查詢裡，前端不用多打一次 API；`ok`/從沒抓過/
  `no_gallery_id`/`unsupported_source` 都不顯示徽章（見 `BP-SVC-DOUJIN-1` 的理由）。
  編輯面板存檔或抓取後，卡片用同一份 `ATTENTION_STATUSES` 邏輯（跟後端
  `doujin_meta_service.ATTENTION_FETCH_STATUSES` 對應，註解互相指到對方）即時重算，
  不必整頁重新整理。
...
牆同樣有對應的窄螢幕收斂（沿用 `GeneralGalleryPanel.vue`）。**已由 Playwright 驗收**
（獨立一輪，非本次開發過程自證）：375/768/1280 三檔位共 10 項檢查全過，含兩個
merge-blocking 項目——一般模式縮圖牆行為不變、三個檔位皆無水平捲軸（封面牆/閱讀器/
編輯面板都在內）。本輪（exhentai 移除、抓取狀態徽章）之後的畫面尚未另外跑一次 Playwright
——新增的 ⚠ 徽章是純 CSS 定位在既有卡片內，風險低，但仍然**沒有**、也**不能宣稱**這次改動
本身「畫面實際渲染正確」，只做了 `npm run build` 成功（無編譯錯誤）。
...
- **exhentai——已決議，不再是開放問題**：使用者 2026-08-26 決定「exhentai 不用 他要專門的
  cookie才能瀏覽」，已從 `DOUJINSHI_SOURCES` 移除（見 `BP-SVC-DOUJIN-1`）。保留在這裡的
  記錄只是說明它曾經是本子模式的一部分、為什麼被拿掉，不是還待決定。
