...
from app.domain.error_sanitizer import sanitize_error as _sanitize_error

# Re-exported (not called directly in this module) purely for backward
# compatibility: tests/test_wnacg_provider.py calls `wnacg._strip_url_query(...)`
# directly as a module attribute (pre-dates this module's sanitizer being
# extracted into app/domain/error_sanitizer.py) — keeping the name bound
# here means those tests need zero changes. ruff cannot see the dynamic
# `wnacg._strip_url_query` access from the test file, so this import reads as
# unused from a purely static analysis; noqa is correct, not a workaround.
from app.domain.error_sanitizer import strip_url_query as _strip_url_query  # noqa: F401
