...
async function selectCategory(cat) {
  ...
  try {
    items.value = await apiRequest(`/api/gallery/items?category=${encodeURIComponent(cat.path)}`)
  } finally {
...
// cleanup listener on unmount omitted for brevity — safe for SPA navigation
</script>
...
<template>
  <section class="page">
    ...
    <!-- Search -->
    <div v-if="selectedCategory" class="gallery-toolbar">
      <input
        v-model="search"
    ...
    <div v-else-if="selectedCategory && !filteredItems.length" class="empty-state">
      此分類沒有項目。
    </div>
